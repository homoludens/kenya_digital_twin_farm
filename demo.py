#!/usr/bin/env python3
"""
Kenya Digital Farm Twin - Demo Version (No PCSE Required)
This version uses simulated data to demonstrate the UI without requiring PCSE installation.
"""
...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QLabel, QComboBox, QDateEdit, QDoubleSpinBox,
    QPushButton, QTabWidget, QGroupBox, QTableWidget, QTableWidgetItem,
    QHeaderView, QScrollArea, QFrame, QSplitter, QMessageBox,
    QProgressBar, QStatusBar, QMenuBar, QMenu, QAction, QDialog,
    QDialogButtonBox, QFormLayout, QSpinBox, QLineEdit, QTextEdit,
    QSizePolicy, QCheckBox
)
from PyQt5.QtCore import Qt, QDate, pyqtSignal, QThread, QSize, QTimer
...
CROP_OPTIONS = {
    'barley': {
        ...
        'season_days': 120,
        'n_demand': 'medium',
        'base_yield': 2.2,  # Base yield in t/ha
        'n_response': 0.008,  # Yield increase per kg N
    },
    'wheat': {
...
SOIL_PARAM_INFO = {
    ...
    'NSOILBASE_FR': ('N Mineralization', 'Fraction of soil N available (fraction)', 0.01, 0.1),
}
...
def generate_simulated_data(crop_name: str, n_rate: float, season_days: int, 
                           soil_modifier: float = 1.0, start_date: date = None) -> pd.DataFrame:
    """Generate simulated crop growth data without PCSE."""
    
    if start_date is None:
        start_date = date(2023, 3, 15)
    
    crop_config = CROP_OPTIONS.get(crop_name, CROP_OPTIONS['barley'])
    base_yield = crop_config.get('base_yield', 2.5) * soil_modifier
    n_response = crop_config.get('n_response', 0.01)
    
    # Calculate final yield with diminishing returns
    final_yield = base_yield + n_response * n_rate * (1 - n_rate / 300)
    final_yield = max(final_yield, base_yield * 0.8)
    
    # Generate daily data
    days = pd.date_range(start=start_date, periods=season_days)
    
    # DVS (Development Stage): 0 -> 1 (flowering) -> 2 (maturity)
    dvs = np.zeros(season_days)
    flowering_day = int(season_days * 0.45)
    maturity_day = season_days - 1
    
    for i in range(season_days):
        if i <= flowering_day:
            dvs[i] = i / flowering_day
        else:
            dvs[i] = 1 + (i - flowering_day) / (maturity_day - flowering_day)
    
    # LAI (Leaf Area Index): bell curve
    max_lai = 3.0 + n_rate * 0.015
    lai_peak = int(season_days * 0.5)
    lai = max_lai * np.exp(-((np.arange(season_days) - lai_peak) / (season_days * 0.25))**2)
    
    # Biomass accumulation (sigmoid)
    max_biomass = final_yield * 2.5 * 1000  # kg/ha
    t = np.arange(season_days)
    biomass = max_biomass / (1 + np.exp(-0.08 * (t - season_days * 0.5)))
    
    # Yield accumulation (starts after flowering)
    yield_data = np.zeros(season_days)
    for i in range(flowering_day, season_days):
        progress = (i - flowering_day) / (season_days - flowering_day)
        yield_data[i] = final_yield * 1000 * progress**1.5
    ...
    return df, final_yield


# =============================================================================
# SIMULATION WORKER THREAD (Demo Version)
# =============================================================================

class SimulationWorker(QThread):
    """Worker thread for running simulated crop simulations."""
    ...
    def run(self):
        try:
            ...
            # Get soil modifier
            soil_modifier = 1.0
            soil_n = soil_params.get('NSOILBASE', 60)
            if soil_n > 80:
                soil_modifier = 1.1
            elif soil_n < 40:
                soil_modifier = 0.85
            ...
            results = []
            dataframes = {}
            total_scenarios = len([s for s in fert_scenarios.values() if s.get('enabled', True)])
            completed = 0
            ...
            for key, scenario in fert_scenarios.items():
                ...
                self.progress.emit(progress_pct, f"Simulating: {scenario['name']}...")
                
                # Generate simulated data
                df, final_yield = generate_simulated_data(
                    crop_name,
                    scenario['total_n'],
                    crop_config['season_days'],
                    soil_modifier,
                    start_date
                )
                
                # Add some random variation
                final_yield *= (0.95 + np.random.random() * 0.1)
                
                results.append({
                    'df': df,
                    'summary': {},
                    'yield_kg': final_yield * 1000,
                    'yield_t': final_yield,
                    'scenario': scenario['name'],
                    'scenario_key': key,
                    'n_rate': scenario['total_n'],
                    'tagp': final_yield * 2.5 * 1000,
                    'laimax': 3.0 + scenario['total_n'] * 0.015,
                })
...
class LocationMapWidget(QWidget):
    ...
    def draw_map(self):
        ...
        kenya_lon = [34, 42, 42, 41, 40, 34, 34]
        kenya_lat = [4.5, 4.5, -1, -4.5, -4.5, -1, 4.5]
        ax.plot(kenya_lon, kenya_lat, 'k-', linewidth=2, alpha=0.5)
        ax.fill(kenya_lon, kenya_lat, color='#e8f5e9', alpha=0.3)
        ...
        for key, loc in LOCATION_OPTIONS.items():
            ...
            ax.scatter(loc['lon'], loc['lat'], c=color, s=size, marker=marker, 
                      edgecolors='white', linewidths=2, zorder=5)
            ax.annotate(loc['name'].split('(')[0].strip(), 
                       (loc['lon'], loc['lat']),
                       xytext=(5, 5), textcoords='offset points',
                       fontsize=8, fontweight='bold' if key == self.selected_location else 'normal')
    ...
    def on_click(self, event):
        ...
        for key, loc in LOCATION_OPTIONS.items():
            dist = ((loc['lon'] - event.xdata)**2 + (loc['lat'] - event.ydata)**2)**0.5
            if dist < min_dist:
                min_dist = dist
                closest = key
        
        if closest and min_dist < 2:
...
class FertilizerSettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(['Enabled', 'Scenario', 'Total N (kg/ha)', 'Applications', 'Description'])
    ...
    def populate_table(self):
        ...
        for row, (key, scenario) in enumerate(self.fert_scenarios.items()):
            ...
            checkbox.stateChanged.connect(lambda state, k=key: self.toggle_enabled(k, state))
            ...
            self.table.setItem(row, 3, QTableWidgetItem(self.format_applications(scenario['applications'])))
...
class ResultsPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.crop_name = 'barley'
        self.location_name = 'Trans Nzoia'
    ...
    def plot_nitrogen_response(self):
        ...
        # Bar chart
        bars = ax1.bar(range(len(df_results)), df_results['yield_t'], color=colors, 
        ...
        ax1.set_title(f'(a) {self.crop_name.capitalize()} Yield by N Rate', fontweight='bold', fontsize=10)
        ax1.set_ylim(0, max(df_results['yield_t']) * 1.25)
        ax1.grid(True, axis='y', alpha=0.3)
        
        # Response curve
        ax2.plot(df_results['n_rate'], df_results['yield_t'], 'o-', color='#27AE60',
                linewidth=2, markersize=10, markeredgecolor='white', markeredgewidth=2)
        ax2.fill_between(df_results['n_rate'], 0, df_results['yield_t'], alpha=0.2, color='#27AE60')
        
        for _, row in df_results.iterrows():
            ax2.annotate(f'{row["yield_t"]:.2f}', xy=(row['n_rate'], row['yield_t']),
    ...
    def plot_crop_growth(self):
        ...
        if 'TAGP' in df.columns and 'TWSO' in df.columns:
            axes[0, 1].fill_between(df['day'], 0, df['TAGP']/1000, color='brown', alpha=0.3, label='Total Biomass')
            axes[0, 1].fill_between(df['day'], 0, df['TWSO']/1000, color='gold', alpha=0.6, label='Storage Organs')
            axes[0, 1].plot(df['day'], df['TAGP']/1000, 'brown', linewidth=2)
            axes[0, 1].plot(df['day'], df['TWSO']/1000, 'gold', linewidth=2)
        ...
        if 'DVS' in df.columns:
            ...
            axes[1, 0].fill_between(df['day'], 0, df['DVS'], where=df['DVS']<=1, color='lightgreen', alpha=0.3)
            axes[1, 0].fill_between(df['day'], 0, df['DVS'], where=(df['DVS']>1) & (df['DVS']<=2), color='lightyellow', alpha=0.3)
            axes[1, 0].set_ylabel('DVS', fontweight='bold')
            axes[1, 0].set_xlabel('Date', fontweight='bold')
            axes[1, 0].set_title('(c) Development Stage', fontweight='bold')
        
        # Simulated N uptake
        n_uptake = np.cumsum(np.random.exponential(0.5, len(df)))
        n_uptake = n_uptake / n_uptake.max() * 80
        axes[1, 1].plot(df['day'], n_uptake, 'g-', linewidth=2, label='N Uptake')
    ...
    def plot_multiyear_analysis(self):
        ...
        # Yield response with variability
        ax1 = axes[0]
        x = df_results['n_rate']
        y = df_results['yield_t']
        
        ax1.plot(x, y, 'o-', color='#3498DB', linewidth=2, markersize=10,
                markeredgecolor='white', markeredgewidth=2, label='Mean Yield')
        
        y_upper = y * 1.1
        y_lower = y * 0.9
        ax1.fill_between(x, y_lower, y_upper, alpha=0.2, color='#3498DB', label='±10% Range')
        
        ax1.set_xlabel('Nitrogen Applied (kg N/ha)', fontweight='bold')
        ax1.set_ylabel('Yield (t/ha)', fontweight='bold')
        ax1.set_title('(a) Yield Response with Variability', fontweight='bold', fontsize=10)
        ...
        ax2.bar(x_pos - width/2, df_results['yield_t'], width, label='Grain Yield', color='#27AE60')
        ax2.bar(x_pos + width/2, df_results['tagp'], width, label='Total Biomass', color='#8E44AD')
        ...
        ax2.set_xticklabels([s.replace(' kg N/ha)', '').replace('(', '\n') for s in df_results['scenario']], 
    ...
    def plot_yield_gap(self):
        ...
        ax.bar(x, df_results['actual_yield'], width, label=f'Actual Yield ({self.yield_gap_factor*100:.0f}%)', 
              color='#27AE60')
        ax.bar(x, df_results['yield_gap'], width, bottom=df_results['actual_yield'],
              label='Yield Gap', color='#E74C3C', alpha=0.7)
        
        ax.plot(x, df_results['yield_t'], 'ko-', linewidth=2, markersize=8, label='Simulated Potential')
        
        for i, (actual, gap, total) in enumerate(zip(df_results['actual_yield'], 
                                                      df_results['yield_gap'],
                                                      df_results['yield_t'])):
            ax.annotate(f'{actual:.1f}', xy=(i, actual/2), ha='center', va='center', 
                       fontsize=8, fontweight='bold', color='white')
            ax.annotate(f'{total:.2f}', xy=(i, total + 0.1), ha='center', va='bottom', 
                       fontsize=8, fontweight='bold')
        
        ax.set_xlabel('Fertilizer Scenario', fontweight='bold')
        ax.set_ylabel('Yield (t/ha)', fontweight='bold')
        ax.set_title(f'Yield Gap Analysis - {self.crop_name.capitalize()}', fontweight='bold', fontsize=12)
        ax.set_xticks(x)
        ax.set_xticklabels(df_results['scenario'], rotation=20, ha='right', fontsize=8)
        ax.legend(loc='upper left', fontsize=9)
        ax.grid(True, axis='y', alpha=0.3)
        
        ax.text(0.98, 0.02, f'Yield Gap Factor: {self.yield_gap_factor:.0%}\n(Actual ÷ Potential)',
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Kenya Digital Farm Twin - WOFOST Simulator (DEMO MODE)")
    ...
    def setup_ui(self):
        ...
        # Demo mode banner
        demo_banner = QLabel("🎮 DEMO MODE - Simulated Data")
        demo_banner.setStyleSheet("background-color: #FFF3CD; color: #856404; padding: 10px; font-weight: bold; border-radius: 5px;")
        demo_banner.setAlignment(Qt.AlignCenter)
        left_layout.addWidget(demo_banner)
        ...
        yield_info = QLabel("Typical smallholder: 0.25-0.40\nCommercial farms: 0.60-0.80")
    ...
    def setup_statusbar(self):
        ...
        self.statusbar.showMessage("Ready - Demo Mode (No PCSE Required)")
    ...
    def show_fert_settings(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            enabled_count = sum(1 for s in self.fert_scenarios.values() if s.get('enabled', True))
            self.statusbar.showMessage(f"Fertilizer scenarios updated: {enabled_count} enabled")
    ...
    def on_simulation_finished(self, results: List, dataframes: Dict):
        ...
        if results:
            ...
            self.statusbar.showMessage(f"Simulation complete: {len(results)} scenarios (Demo Mode)")
        else:
    ...
    def show_about(self):
        QMessageBox.about(self, "About Kenya Digital Farm Twin",
            "<h2>Kenya Digital Farm Twin - DEMO</h2>"
            "<p>WOFOST Crop Simulation Application</p>"
            "<p><b>This is a DEMO version</b> that uses simulated data.</p>"
            "<p>For full functionality, install PCSE and use main.py</p>"
            "<hr>"
            "<p><b>Features:</b></p>"
            "<ul>"
