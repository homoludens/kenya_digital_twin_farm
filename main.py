        ...
        try:
            ...
            for key, scenario in fert_scenarios.items():
                ...
                if scenario['applications']:
                    timed_events = []
                    for i, (days, amount, recovery) in enumerate(scenario['applications']):
...
class FertilizerSettingsDialog(QDialog):
    ...
    def edit_scenario(self):
        ...
        QMessageBox.information(self, "Edit", f"Edit scenario: {key}\n(Full editor to be implemented)")
        
    def add_scenario(self):
        # Simplified - would normally show a dialog
        QMessageBox.information(self, "Add", "Add new scenario\n(Full editor to be implemented)")
...
class ResultsPanel(QWidget):
    ...
    def update_results(self, results: List, dataframes: Dict, yield_gap_factor: float = 0.35, crop_name: str = 'barley', location_name: str = 'Trans Nzoia'):
    ...
    def plot_crop_growth(self):
        ...
        if 'DVS' in df.columns:
            ...
            axes[1, 0].text(df['day'].iloc[3*len(df)//4], 1.5, 'Reproductive', fontsize=8)
        
        # Water/N stress (if available)
        stress_cols = ['RFTRA', 'NNI']
        stress_found = False
        for col in stress_cols:
            if col in df.columns:
                axes[1, 1].plot(df['day'], df[col], linewidth=2, label=col)
                stress_found = True
        
        if stress_found:
            axes[1, 1].set_ylabel('Stress Index', fontweight='bold')
            axes[1, 1].set_title('(d) Stress Indices', fontweight='bold')
            axes[1, 1].legend(fontsize=7)
        else:
            axes[1, 1].text(0.5, 0.5, 'Stress data\nnot available', ha='center', va='center', transform=axes[1, 1].transAxes)
            axes[1, 1].set_title('(d) Stress Indices', fontweight='bold')
    ...
    def plot_multiyear_analysis(self):
        ...
        bars1 = ax2.bar(x_pos - width/2, df_results['yield_t'], width, label='Grain Yield', color='#27AE60')
        bars2 = ax2.bar(x_pos + width/2, df_results['tagp'], width, label='Total Biomass', color='#8E44AD')
    ...
    def plot_yield_gap(self):
        ...
        # Stacked bar chart
        bars1 = ax.bar(x, df_results['actual_yield'], width, label=f'Actual Yield ({self.yield_gap_factor*100:.0f}%)', 
                      color='#27AE60')
        bars2 = ax.bar(x, df_results['yield_gap'], width, bottom=df_results['actual_yield'],
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.yield_gap_spin.setToolTip("Ratio of actual farmer yield to simulated potential yield")
